...
function createMockConfig(overrides?: Partial<Strategy2Config>): Strategy2Config {
	return {
		...
		parameters: {
			housing_growth: {
				segment: 'housing',
				description: 'Housing growth',
				operation: 'multiply' as const,
				values: [
					{ index: 0, label: 'Baseline', value: 0, hasData: true },
					{ index: 1, label: 'High', value: 10, hasData: true },
					{ index: 2, label: 'Low', value: 20, hasData: false }
				]
			},
			transport_flex: {
				segment: 'transport',
				description: 'Transport flexibility',
				operation: 'multiply' as const,
				values: [
					{ index: 0, label: 'Baseline', value: 0, hasData: true },
					{ index: 1, label: 'High', value: 10, hasData: true }
...
describe('parameterStore', () => {
	...
	test('initialize sets config and defaults', () => {
		...
		expect(parameterStore.config).toEqual(config);
...
describe('getParameterLabel', () => {
	...
	test('returns fallback for unknown index', () => {
		const param: Strategy2Parameter = {
			...
			description: 'Test',
			operation: 'multiply' as const,
			values: [{ index: 0, label: 'Baseline', value: 0, hasData: true }]
		};
...
describe('parameterHasData', () => {
	...
	test('returns false when value has no data', () => {
		const param: Strategy2Parameter = {
			...
			description: 'Test',
			operation: 'multiply' as const,
			values: [{ index: 0, label: 'Baseline', value: 0, hasData: false }]
		};
